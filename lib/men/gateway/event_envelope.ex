defmodule Men.Gateway.EventEnvelope do
  ...
  defp normalize_task_payload(payload) when is_map(payload) do
    with {:ok, normalized} <- stringify_keys(payload),
         ...
         {:ok, schedule_id} <- fetch_optional_text(normalized["schedule_id"], :schedule_id),
         {:ok, fire_time} <- fetch_optional_occurred_at(normalized["fire_time"], :fire_time),
         :ok <- validate_schedule_instance_fields(schedule_id, fire_time),
      ...
      payload =
        ...
        |> maybe_put(:schedule_id, schedule_id)
        |> maybe_put(:fire_time, fire_time)
  ...
  defp fetch_optional_occurred_at(nil, _field), do: {:ok, nil}

  defp fetch_optional_occurred_at(value, field) do
    case fetch_occurred_at(value) do
      {:ok, normalized} -> {:ok, normalized}
      {:error, _} -> {:error, {:invalid_field, field}}
    end
  end

  defp validate_schedule_instance_fields(nil, nil), do: :ok

  defp validate_schedule_instance_fields(schedule_id, fire_time)
       when is_binary(schedule_id) and is_binary(fire_time),
       do: :ok

  defp validate_schedule_instance_fields(nil, _fire_time),
    do: {:error, {:invalid_field, :schedule_id}}

  defp validate_schedule_instance_fields(_schedule_id, nil),
    do: {:error, {:invalid_field, :fire_time}}
  ...
  defp normalize_task_event_ets_keys(ets_keys, payload) do
    ...
    base_keys
    |> maybe_append(payload[:schedule_id])
    |> maybe_append(payload[:fire_time])
    |> Enum.uniq()
  end
  ...
  defp maybe_append(list, value), do: [value | list]
end
