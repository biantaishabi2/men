...
不是所有事件都要持久化，建议按“状态 vs 信号”分层：

1. 必须持久化（可恢复状态）
- 共享状态区中的关键数据：任务结果、证据、控制快照、关键 inbox 记录。
- 目标：主 Agent 或节点重启后可恢复连续性。
...
3. 建议持久化分类
- 关键事件（落盘/可回放）：`agent_result`、`agent_error`、`policy_changed`。
- 噪声事件（内存/TTL）：`heartbeat`、`tool_progress`、`telemetry_update`。
