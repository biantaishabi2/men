defmodule Men.Integration.DingtalkWebhookFlowTest do
    ...
    def start_turn(prompt, context) do
      ...
      case decode_payload(prompt) do
        %{"content" => content} when is_binary(content) ->
          cond do
            String.contains?(content, "session_not_found_once") ->
            ...
            String.contains?(content, "bridge_error") ->
              {:error,
               %{
                 type: :failed,
                 code: "BRIDGE_FAIL",
  ...
  test "连续 webhook 对话复用 runtime_session_id", %{conn: conn} do
    payload_1 =
      payload("evt-integration-1", "user-integration", "conv-integration", "@bot turn-1")

    payload_2 =
      payload("evt-integration-2", "user-integration", "conv-integration", "@bot turn-2")
  ...
  test "不同 session_key 并发 webhook 隔离", %{conn: conn} do
    payload_1 = payload("evt-integration-3", "user-a", "conv-a", "@bot parallel-a")
    payload_2 = payload("evt-integration-4", "user-b", "conv-b", "@bot parallel-b")
  ...
  test "bridge 失败不影响 webhook 立即 ACK，后台按既有语义 error 回写", %{conn: conn} do
    event_payload = payload("evt-integration-6", "user-error", "conv-error", "@bot bridge_error")
  ...
  test "非@消息不回复且不触发 bridge", %{conn: conn} do
    event_payload = payload("evt-integration-7", "user-ignore", "conv-ignore", "你好")
